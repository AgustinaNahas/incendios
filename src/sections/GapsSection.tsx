...
export function GapsSection() {
  return (
    <StorySection
      ...
      graphic={
        <div>
          <p className="mb-3 text-xs font-semibold tracking-wide uppercase opacity-70">
    ...
    >
      <div className="overflow-x-auto">
        <table className="w-full min-w-[520px] border-collapse text-left text-sm">
          <thead>
            <tr className="border-b border-current/25 text-xs uppercase tracking-wide opacity-70">
          ...
          <tbody>
            {gaps.map((g) => (
              <tr key={g.datum} className="border-b border-current/10 align-top">
                <td className="py-3 pr-3 font-medium">{g.datum}</td>
                <td className="py-3 pr-3 opacity-80">{g.status}</td>
                <td className="py-3 opacity-80">{g.impact}</td>
              </tr>
            ))}
          </tbody>
        </table>
